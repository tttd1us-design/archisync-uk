...
export default function LiveInterpreter({ 
  ...
  // 📜 Archive ON/OFF Toggle State (이전 대화 백업 아카이브 표시/숨김)
  const [showArchive, setShowArchive] = useState(() => {
    try {
      const saved = localStorage.getItem('archisync_show_archive');
  ...
  const toggleArchive = () => {
    setShowArchive(prev => {
      const next = !prev;
      try { localStorage.setItem('archisync_show_archive', String(next)); } catch {}
      return next;
    });
  };
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4.5rem)] max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5 gap-2.5">
        ...
        <div className={`flex flex-wrap items-center justify-between pb-2.5 mb-3 border-b ${
          ...
          <div className="flex flex-wrap items-center gap-1.5 relative">
            ...
            <button
              ...
              title={showArchive ? "하단 이전 대화 백업 아카이브 숨기기 (무대 전체화면)" : "하단 이전 대화 백업 아카이브 펼치기"}
            >
      ...
      {/* 📜 SESSION BACKUP HISTORY ARCHIVE (Collapsible ON/OFF Container) */}
      {showArchive ? (
        <div className={`flex-1 ${
          isDark ? 'bg-slate-900/90 border-slate-800 text-slate-100' : 'bg-slate-50 border-slate-200 text-slate-900'
        } border rounded-xl p-2 overflow-y-auto space-y-1.5 shadow-inner transition-all animate-fadeIn`}>
          ...
          <div className={`flex items-center justify-between pb-1 border-b text-[8.5pt] ${
            ...
            <div className="flex items-center space-x-1.5">
              ...
              {/* 🔼 Collapse / Hide Archive Button */}
              <button
                onClick={toggleArchive}
                className={`text-[8pt] font-bold flex items-center gap-1 px-2 py-0.5 rounded border transition ${
                  isDark 
                    ? 'text-slate-400 hover:text-slate-200 bg-slate-800/60 border-slate-700 hover:border-slate-600' 
                    : 'text-slate-600 hover:text-slate-900 bg-slate-100 border-slate-300 hover:border-slate-400'
                }`}
                title="아카이브 접기 (실시간 무대 집중)"
              >
        ...
        /* 📜 Collapsed Archive Mini Bar (아카이브가 꺼졌을 때 간결한 원클릭 펼침 바) */
        <div
          onClick={toggleArchive}
          className={`cursor-pointer py-2 px-3 rounded-xl border flex items-center justify-between text-[8pt] font-bold transition shadow-xs ${
            isDark 
              ? 'bg-slate-900/80 hover:bg-slate-800/80 border-slate-800 hover:border-indigo-500/40 text-slate-300' 
              : 'bg-white hover:bg-slate-50 border-slate-200 hover:border-indigo-300 text-slate-700'
          }`}
          title="클릭하여 이전 대화 백업 아카이브를 다시 펼칩니다"
        >
          <div className="flex items-center space-x-2">
            <BookOpen className="w-3.5 h-3.5 text-indigo-400" />
            <span>이전 대화 백업 아카이브</span>
            <span className={`px-2 py-0.5 rounded-full text-[7pt] font-mono ${
              isDark ? 'bg-slate-800 text-slate-400' : 'bg-slate-100 text-slate-600'
            }`}>
              {messages.length}건 보관 중
            </span>
          </div>

          <div className="flex items-center space-x-1.5 text-indigo-400 hover:text-indigo-300">
            <Eye className="w-3.5 h-3.5" />
            <span>아카이브 펼치기 (ON)</span>
